...
```bash
cd <workdir> && claude -p "把当前任务转交给外部 Claude CLI 非交互执行"
```

验收标准：

- 命令以 `cd <workdir> && claude -p "` 开头
- 不补解释性文字
...
```bash
cd packages/demo && claude -p "执行用户指定的单一任务"
```
...
```bash
cd <workdir> && claude -p "让 Claude 输出 \"done\" 并打印 \$HOME"
```
