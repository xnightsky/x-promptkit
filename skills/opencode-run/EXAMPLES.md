...
- “用 opencode run 帮我包一条命令，把这个任务转出去”
- “只给我命令，不要执行”
...
```bash
cd <workdir> && opencode run "把当前任务转交给外部代理执行"
```
...
- “在 `packages/demo` 里给我一条 opencode run 命令”
...
```bash
cd packages/demo && opencode run "执行用户指定的单一任务"
```
...
```bash
cd <workdir> && opencode run "让外部代理输出 \"done\" 并打印 \$HOME"
```
...
- “就执行这条：`cd worktree && opencode run \"原样处理这个任务\"`”
