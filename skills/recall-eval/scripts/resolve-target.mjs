#!/usr/bin/env node
// skills/recall-eval/scripts/resolve-target.mjs
//
...
//
// 用法：
//   node skills/recall-eval/scripts/resolve-target.mjs <yaml-path|target-path> [--case <id>]
...
if (!yamlPath) {
  console.log(
    "Usage: node skills/recall-eval/scripts/resolve-target.mjs <yaml-path|target-path> [--case <id>]",
  );
  process.exit(1);
}
