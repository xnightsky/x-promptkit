...
- installer: `skills-def/codex-bridge-minimax-worker-installer`
- loader: `skills-def/codex-bridge-minimax-worker-loader`

vendored bridge 源码位于：

```text
skills-def/codex-bridge-minimax-worker-installer/vendor/codex-bridge
```
...
如果还没启动，它会以分离会话方式在后台拉起：

```bash
node main.mjs --host 127.0.0.1 --port 54187
```

如果当前占用端口的是受管 bridge，但健康检查不过，loader 会先杀掉旧 bridge 再拉起新的实例。
