...
2. **model**：给了 `--model` → 原样用（模糊名先 `cursor-agent --list-models <pattern>` 解析）；没给 → 读套餐默认档（可选，按 `${CLAUDE_PLUGIN_ROOT}/references/scoping.md`「读方」）——读 `${CLAUDE_PLUGIN_DATA}/cursor.yaml` 取 `default:true` 档的 `model`（命中且有 `descr` → 回显）；**读不到（没跑过 `/dev:scope cursor`）→ 回退「日常」档 `composer-2.5-fast`（Cursor 自家编码模型、账号默认），不停**。推理档编进 model ID（见 backends.md#cursor-agent）。
3. **安全档（默认安全，危险档需显式）**：
