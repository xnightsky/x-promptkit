---
description: 把当前任务交接给 pi CLI 加速落地（走统一 dev-run 编排，默认档可选读 .dev-run.yaml 套餐）
...
3. **没给旋钮 → 读套餐默认档（可选）**：按 `${CLAUDE_PLUGIN_ROOT}/references/scoping.md`「读方」——从执行命令的 `PWD` 逐级向 home 检索最近的 `.dev-run.yaml`，取 `backends.pi` 中 `default:true` 档的 `model` + `knobs.thinking`（命中且有 `descr` → 回显）；**所有候选位置都读不到或命中配置没有 pi section → 回退裸 `pi -p`（pi 自身默认 model），不停**。
