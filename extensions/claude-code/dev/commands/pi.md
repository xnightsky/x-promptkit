...
3. **没给旋钮 → 读套餐默认档（可选）**：按 `${CLAUDE_PLUGIN_ROOT}/references/scoping.md`「读方」——就近读 `.dev-run.yaml`（先 `<项目根>` 后 `~`）的 `backends.pi` 取 `default:true` 档的 `model` + `knobs.thinking`（命中且有 `descr` → 回显）；**读不到（没跑过 `/dev:scope pi`）→ 回退裸 `pi -p`（pi 自身默认 model），不停**。
